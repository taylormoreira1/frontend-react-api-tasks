...
import { Heading, Flex, Input, Button, Text, IconButton, VStack, Box, Spacer, Textarea, useToast } from "@chakra-ui/react";
...
export default function Task() {
    ...
    const handleCreateTask = () => {
        ...
        async function createTask() {
            ...
            try {
                ...
                const response = await Api.post("/tasks/", data, config);
    ...
    const handleUpdateTask = async () => {
        try {
            ...
            await Api.put("/tasks/" + selectedTask.id, data, config);
        ...
        <Flex direction="column" minH="100vh" maxW="100%" bg="gray.100">
            ...
            <Flex justify="center" maxW="100%">
                <Flex direction="column" my="10" w="full" maxW="2xl">
                    <Box bg="white" p="5" borderWidth="1px" borderRadius="md" boxShadow="sm">
                    ...
                    <VStack spacing={4} align="stretch" pt="5">
                        {tasks.map(task => (
                            <Box key={task.id} p={4} borderWidth="1px" borderRadius="md" boxShadow="sm" bg="white">

                                <Flex justify="space-between" align="center">
                                    <VStack align="start" spacing={1} flexGrow={1}>
                                        <Text fontWeight="bold">{task.name}</Text>
                                        <Text>{task.description}</Text>
                                        <Text>Data de Entrega: {task.deadline ? new Date(task.deadline).toLocaleString('pt-BR') : "Prazo não definido"}</Text>
                                    </VStack>
                                    <Spacer />
                                    <Flex>
                                        <IconButton
